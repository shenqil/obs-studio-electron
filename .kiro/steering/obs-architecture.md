...
## 代码审核结论与技术债（src/main/obs）

整体健康：分层（api→module→common）严格单向、module 不监听生命周期事件、销毁链各节点 `try/finally` 无条件发 `*:destroyed`、`onAll` join 防互锁；延迟选中在「删源/选中/清空/lifecycle:destroy」均调 `cancelPendingSelection()`，无悬挂定时器。settings/safe/utils 等基础设施有空值与异常兜底。

待清理项（均为低风险、非阻塞）：

- **扬声器自持 fader**：`module/speaker.ts` 直接 `FaderFactory.create` 自管推子（因它非场景项、不能用按场景项 id 索引的 `fader` 模块），导致音量 clamp/容错逻辑与 `fader` 模块小幅重复——有意为之，改动需保持两处一致。

已修正项（上一轮审核）：

- macOS 扬声器创建参数精简为仅 `{ capture_audio: true }`（移除沿用自 mac_screen_capture 的 `show_cursor/type`）。
- `source.onLifecycleDestroy` 增加 `emitSourcesChanged.cancel()`，避免节流广播在销毁后尾触发。
- 删除 `module/speaker.ts` 的死代码 `isCreated()`。
- `window.switchDevice` 在 macOS 切换后校验 `isWindowBound`，与 `createInput` 一致。
- `Global.setOutputSource(ch, null)` 清空通道的类型绕过收敛到 `module/speaker.ts` 私有 `setChannelSource(channel, source|null)` 一处（`NodeObs` 本身为 `any`，设备枚举的返回 `as` 转换不可避免、保留）。
